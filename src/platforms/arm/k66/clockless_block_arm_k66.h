...
public:
	...
	template<int BITS,int PX> __attribute__ ((always_inline)) inline static void writeBits(uint32_t & next_mark, Lines & b, PixelController<RGB_ORDER, LANES, LANE_MASK> &pixels) { // , uint32_t & b2)  {
		Lines b2;
		if(USED_LANES>8) {
